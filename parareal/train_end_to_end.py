...
from models.utils import fetch_data_end_to_end, setup_logger
from parareal.parallel_scheme_training import optimize_solution
from parareal.param_settings import get_params, get_paths
from parareal.visualize_training import visualize_wavefield


def train_Dt_end_to_end(
    logging_bool=False, visualize=True, vis_param=1, params="0", vis_save=True
):
    """
    Parameters
    ----------
    ...
    logging.info(
        " ".join(
            [
                "param settings:",
                ", ".join([i + ": " + str(v) for i, v in param_dict.items()]),
            ]
        )
    )
    ...
    model = Model_end_to_end(
        param_dict, "Interpolation", "UNet3", res_scaler, model_res
    ).double()
    model = torch.nn.DataParallel(model).to(device)  # multi-GPU use
    model.load_state_dict(
        torch.load(
            "../results/run_3/good_one/saved_model_Interpolation_UNet3_AdamW_SmoothL1Loss_2_128_False_15.pt"
        )
    )
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            loss_list = optimize_solution(
                model, loss_f, data, label_distr_shift
            )  # b x 3 x w x h
        ...
        with torch.no_grad():
                ...
                for input_idx in range(n_snaps - 1):
                    ...
                    for label_idx in range(input_idx + 1, n_snaps):
                        ...
                        input_tensor = torch.cat(
                            (output, torch.unsqueeze(input_tensor[:, 3, :, :], dim=1)),
                            dim=1,
                        )
            ...
            if logging_bool:
                ...
                valid_logger.add_scalar(
                    "loss", np.array(val_loss_list).mean(), global_step=global_step
                )
