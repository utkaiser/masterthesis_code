...
from utils_wave import WaveEnergyComponentField_end_to_end, crop_center, get_datagen_end_to_end_params, \
    start_logger_datagen_end_to_end, get_model
...
def generate_wave_from_medium(output_path, res, boundary_condition, visualize, index, n_it, optimization, n_opti=2):
    ...
    total_time, delta_t_star, f_delta_x, f_delta_t, n_snaps = get_datagen_end_to_end_params(param_dict)
    model = get_model(res, boundary_condition, param_dict)
    ...
    for it in range(n_it):
        ...
        u_0, ut_0 = initial_condition_gaussian(vel, res, boundary_condition, mode="generate_data", res_padded=res_padded)

        for k in range(n_opti+1):
            ...
            u_elapse, ut_elapse = u_0.clone(), ut_0.clone()
...
if __name__ == "__main__":
    ...
    for index in range(1):
        generate_wave_from_medium(output_path = "../data/end_to_end_" + str(index),
                                  ...
                                  optimization = "none")  # "parareal", "procrustes"
