...
def generate_wave_from_medium(input_path, output_path, init_res_f = 128, absorbing_bc = False, visualize=False):
    ...
    # n_it x n_snaps w x h    ->    200 x 10 x 128 x 128
    ...
    f_delta_x =  2./128.0   #discretization in spatial (fine discretization, fine solver)
    f_delta_t = f_delta_x / 40  #discretization in time (fine discretization, fine solver)
    ...
    for it in range(n_it):
        ...
        vel = crop_center(vel, res_f, res_f)
        ...
        if absorbing_bc:
            ...
            vel_crop = crop_center(vel, init_res_f, init_res_f)
        ...
        for s in range(1, n_snaps+1):
            ...
            if visualize: visualize_wavefield(
                (WaveEnergyComponentField_end_to_end(u_elapse, ut_elapse, vel, f_delta_x)), vel=vel, init_res_f=init_res_f, frame=True)
