...
def crop_center_tensor(img, crop_size, scaler=2):
    """
    Parameters
    ----------
    img : (numpy / pytorch tensor) input image to crop
...
def get_resolution_padding(resolution, optimization):
    """
    Parameters
    ----------
...
def get_wavefield(
    wave_representation, vel, delta_x=2.0 / 128.0, delta_t=(2.0 / 128.0) / 20
):
    """
    Parameters
    ----------
    ...
    u_x, u_y, u_t_c = (
        wave_representation[:, 0],
        wave_representation[:, 1],
        wave_representation[:, 2],
    )
    u, u_t = WaveSol_from_EnergyComponent_tensor(
        u_x, u_y, u_t_c, vel, delta_t, torch.sum(torch.sum(torch.sum(u_x)))
    )
    return (
        WaveEnergyField_tensor(
            u.squeeze().cpu(), u_t.squeeze().cpu(), vel.squeeze().cpu(), delta_x
        )
        * delta_x
        * delta_x
    )
...
def get_wavefield_numpy(
    wave_representation, vel, f_delta_x=2.0 / 128.0, f_delta_t=(2.0 / 128.0) / 20
):
    """
    Parameters
    ----------
    ...
    u, u_t = WaveSol_from_EnergyComponent(
        u_x, u_y, u_t_c, vel, f_delta_t, np.sum(np.sum(np.sum(u_x)))
    )
    return WaveEnergyField(u, u_t, vel, f_delta_x) * f_delta_x * f_delta_x


def smaller_crop(tensor):
    """
    Parameters
    ----------
