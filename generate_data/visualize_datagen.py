...
def visualize_wavefield(
    ...
    init_res_f = 128
    wave_field = WaveEnergyComponentField_end_to_end(
        u_elapse, ut_elapse, vel, f_delta_x
    )
    ...
    plt.plot(
        [axis_center - init_res_f // 2, axis_center + init_res_f // 2],
        [axis_center + init_res_f // 2, axis_center + init_res_f // 2],
        "r",
        linewidth=4,
    )
    ...
    __ = ax2.imshow(
        WaveEnergyField(u, ut.numpy(), vel.numpy(), f_delta_x) * f_delta_x * f_delta_x
    )
    plt.plot(
        [axis_center - init_res_f // 2, axis_center + init_res_f // 2],
        [axis_center + init_res_f // 2, axis_center + init_res_f // 2],
