...
def initial_condition_gaussian(vel, resolution, optimization, mode, res_padded):
    """
    Parameters
    ----------
...
def _get_init_cond_settings(resolution, optimization):
    """
    Parameters
    ----------
    resolution : (int) resolution of actual area to propagate wave
    ...
    center_x, center_y = ((factor_center_x * 0.45) / 2) / factor_start_point_wave, (
        (factor_center_y * 0.45) / 2
    ) / factor_start_point_wave
    if resolution == 128:
        ...
        raise NotImplementedError("Parameter for initial condition not implemented.")
