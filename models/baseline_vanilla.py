...
from models.utils import (
    ...
    vanilla_benchmark_grid_search_end_to_end,
)


def get_results_vanilla(experiment_index=0, flipping=False, model_res=128):
    ...
    for dx, dt in vanilla_benchmark_grid_search_end_to_end(param_d, experiment_index):
        ...
        logging.info(
            f"Baseline_{param_d['c_delta_x']:.5f}_{param_d['c_delta_t']:.5f} has {val_performance} compared to best performance so far {dict_best_score['score']}"
        )

        if dict_best_score["score"] > val_performance:
            dict_best_score = {"score": val_performance, "dx": dx, "dt": dt}
    logging.info(
        f"Best overall performance is {dict_best_score['score']} using baseline_{dict_best_score['dx']:.5f}_{dict_best_score['dt']:.5f}"
    )
...
def val_model_vanilla(model, val_loader, loss_f):
    val_loss_list = []
...
if __name__ == "__main__":
    get_results_vanilla(experiment_index=0)
