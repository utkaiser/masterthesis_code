...
def train_Dt_old_paper(
    ...
    for lr, batch_size, weight_decay, _, _ in hyperparameter_grid_search_end_to_end(experiment_index, param_d, "old_paper"):
        ...
        for epoch in range(param_d["n_epochs"]):
            ...
            train_loss_list, model, global_step, optimizer = train_model(model, train_loader, param_d, global_step,optimizer, loss_f)
            val_performance = val_model(model, vis_path, vis_save,val_loader, loss_f, epoch, train_loss_list, visualize_res_bool)
    ...
    for epoch in range(param_d["n_epochs"]):
        train_loss_list, model, global_step, optimizer = train_model(model, trainval_loader, param_d, global_step, optimizer, loss_f)
        _ = val_model(model, vis_path, vis_save, test_loader,loss_f, epoch, train_loss_list, visualize_res_bool)
...
def train_model(model, train_loader, param_d, global_step, optimizer, loss_f):
    ...
    for i, data in enumerate(train_loader):
        ...
        for input_idx in random.choices(range(param_d["n_snaps"]+1), k=param_d["n_snaps"]):
            ...
            input_tensor = resize_to_coarse(data[:, input_idx, :4].detach(), 64)  # b x 4 x w x h
            ...
            visualize_model_effect(data[:, input_idx, -1].detach().cpu(), input_tensor.detach().cpu(), output.detach().cpu(), label.detach().cpu(), True)

    return train_loss_list, model, global_step, optimizer


def val_model(model, vis_path, vis_save, val_loader, loss_f,
        ...
        for _, path in enumerate(data_paths):
            ...
            datasets.append(
                torch.utils.data.TensorDataset(
                    torch.stack((pad_matrix(torch.from_numpy(np_array['Ux_coarse'])),
