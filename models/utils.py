...
def sample_label_normal_dist(
    input_idx, n_snaps, label_distr_shift, multi_step, weighted_loss
):
    """
    Parameters
    ----------
...
def setup_logger(
    logging_bool, train_logger_path, valid_logger_path, model_name, model_res, vis_path
):
    """
    Parameters
    ----------
...
def choose_optimizer(name, model, lr, weight_decay):
    """
    Parameters
    ----------
...
def hyperparameter_grid_search_end_to_end(
    ...
    list_c_delta_x = [2**-6, 2**-5, 2**-4]

    list_c_delta_t = [2**-10, 2**-9, 2**-8]

    list_all_allocations = []

    if experiment_index == 0:
        return [(0.001, 2**6, 0.01, param_d["c_delta_x"], param_d["c_delta_t"])]
            ...
            for bs in list_batch_size:
                for wd in list_weight_decay:
                    if experiment_index != 2 or suffix == "old_paper":
                        list_all_allocations.append(
                            (lr, bs, wd, param_d["c_delta_x"], param_d["c_delta_t"])
                        )
                    else:
                        # EXPERIMENT 2
...
def vanilla_benchmark_grid_search_end_to_end(param_d, experiment_index):
    list_all_allocations = []
