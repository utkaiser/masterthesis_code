...
from models.optimization_models.utils_optimization import smaller_crop, one_iteration_pseudo_spectral_tensor
...
def parareal_scheme(model, input_idx, n_parareal, label_range, loss_f, data):
    ...
    for k in range(1,n_parareal+1):
        ...
        res_fine, res_model = get_optimizing_terms(model, big_tensor[k-1], vel, input_idx, label_range)  # n_snaps x b x c x w x h
    ...
    plot_big_tensor(smaller_crop(big_tensor[:,:,1:]), smaller_crop(vel), smaller_crop(data[:,input_idx + 1:label_range+1]))
